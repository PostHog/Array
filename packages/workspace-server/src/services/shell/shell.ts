...
function buildShellEnv(
  ...
  // The workspace-server runs with ELECTRON_RUN_AS_NODE=1 and
  // POSTHOG_CODE_INTERNAL_CHILD=1, but user-facing terminals must not inherit
  // either: ELECTRON_RUN_AS_NODE makes any Electron-based CLI run as node, and
  // POSTHOG_CODE_INTERNAL_CHILD causes the bootstrap guard to refuse direct
  // invocations of the app binary. The PATH node shim no longer needs the
  // ambient var (it is self-contained).
  delete env.ELECTRON_RUN_AS_NODE;
  delete env.POSTHOG_CODE_INTERNAL_CHILD;
