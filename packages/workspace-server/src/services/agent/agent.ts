...
- Pull: \`git fetch origin ${context.branchName} && git merge FETCH_HEAD\``;
  ...
  async getPreviewConfigOptions(
    ...
    const gatewayUrl = getLlmGatewayUrl(apiHost);
    // Authed so the gateway can mark plan-restricted models; falls back to an
    // anonymous fetch (everything allowed) when auth isn't available.
    ...
    // Never preselect a model the org's plan can't use — a free-tier org
    // would 403 on its first message.
