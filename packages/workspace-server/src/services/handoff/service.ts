...
export class HandoffHostService implements HandoffHost {
  ...
  async syncCloudCheckpoints(
    ...
    // Fetch the authoritative run log from S3. (The old desktop path preferred a
    // local seeded cache, but the workspace-server log gateway exposes no raw
    // reader — and the local seed can be absent/unflushed when this runs, which
    // silently dropped every cloud checkpoint ref and made restore fail with
    // "Checkpoint not found". S3 is the source of truth the saga already proved
    // fetchable via resumeFromLog.)
    let entries: AgentTypes.StoredEntry[] = [];
    try {
      const taskRun = await apiClient.getTaskRun(taskId, runId);
      entries = await apiClient.fetchTaskRunLogs(taskRun);
    } catch (err) {
      throw new Error(
        `Failed to fetch run log for cloud checkpoint sync: ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
    }

    // Only process checkpoints with S3 artifact paths (cloud captures). Local
    // captures were already materialized during apply_git_checkpoint.
    ...
    for (const event of checkpointEvents) {
      try {
        // Recreate the checkpoint git ref so RevertCheckpointSaga can resolve it.
        // Non-destructive: the caller's apply_git_checkpoint step already set the
        // final working tree, so this only unpacks objects and rebuilds the ref.
        await tracker.materializeCheckpointRef(event, {
          localGitState: undefined,
        });

        // Register in the agent service so the restore button appears for this
        // cloud turn immediately.
        ...
        // the handoff. That turn simply won't be restorable.
      }
    }
  }
  ...
  async uploadPriorLocalCheckpoints(
    ctx: HandoffApiContext,
    taskId: string,
    runId: string,
    repoPath: string,
