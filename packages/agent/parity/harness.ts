...
function redact(value: any): any {
  ...
  for (const [k, v] of Object.entries(value)) {
    if (k === "sessionId") out[k] = "<id>";
      ...
      out[k] = v.map((o: any) => ({ id: o?.id, category: o?.category, value: o?.value, options: (o?.options ?? []).map((x: any) => x?.id ?? x?.optionId) }));
    } else if (k === "modes") {
      out[k] = { currentModeId: (v as any)?.currentModeId, availableModes: ((v as any)?.availableModes ?? []).map((m: any) => m?.id) };
    } else if (k === "usage" && v && typeof v === "object") {
      out[k] = Object.fromEntries(Object.entries(v).map(([uk, uv]) => [uk, typeof uv === "number" ? (uv > 0 ? ">0" : 0) : uv]));
    } else if (typeof v === "string" && v.length > 120) out[k] = `<str:${v.length}>`;
    else out[k] = v;
  }
  return out;
}

export async function runScenario(mode: AdapterMode, scenario: Scenario, cfg: HarnessConfig): Promise<CapturedRun> {
  ...
  const captured: CapturedRun = { adapter: mode, scenario: scenario.name, events: [], stepResults: [] };
  let ord = 0;

  const client = {
    async sessionUpdate(p: any): Promise<void> {
      captured.events.push({ t: ord++, kind: "sessionUpdate", sessionUpdate: p?.update?.sessionUpdate, data: p?.update });
    },
    async requestPermission(p: any): Promise<any> {
      captured.events.push({ t: ord++, kind: "requestPermission", data: { title: p?.toolCall?.title, kind: p?.toolCall?.kind, options: (p?.options ?? []).map((o: any) => ({ id: o?.optionId, kind: o?.kind })) } });
      const allow = (p?.options ?? []).find((o: any) => o?.kind === "allow_once" || o?.kind === "allow_always") ?? p?.options?.[0];
    ...
    async extNotification(method: string, params: any): Promise<void> {
      captured.events.push({ t: ord++, kind: "extNotification", op: method, data: redact(params) });
    },
    async extMethod(method: string, params: any): Promise<any> {
      captured.events.push({ t: ord++, kind: "extMethod", op: method, data: redact(params) });
      return {};
    },
  };

  const acp = createAcpConnection({ adapter: "codex", codexOptions: cfg.codexOptions as any, logger: cfg.logger });
  const stream = ndJsonStream(acp.clientStreams.writable, acp.clientStreams.readable);
  ...
  const ctx: ScenarioCtx = {
    ...
    async step(op, fn) {
      ...
      try {
        ...
        console.error(`  [step] ${op} ✗ (${Date.now() - started}ms): ${String(e?.message ?? e)}`);
        captured.stepResults.push({ op, ok: false, error: String(e?.message ?? e) });
        throw e;
      }
    },
  };

  const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error(`scenario timeout after ${cfg.timeoutMs ?? 180000}ms`)), cfg.timeoutMs ?? 180000));
  try {
    await ctx.step("initialize", () => conn.initialize({ protocolVersion: 1, clientCapabilities: { fs: { readTextFile: true, writeTextFile: true } } }));
