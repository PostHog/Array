...
/**
 * Fail-loud precondition for the live e2e suite. Without POSTHOG_CODE_E2E_PERSONAL_API_KEY every
...
describe("live e2e preconditions", () => {
  it("requires POSTHOG_CODE_E2E_PERSONAL_API_KEY (else the suite would skip-to-green)", () => {
    expect(
      E2E.hasToken,
      "POSTHOG_CODE_E2E_PERSONAL_API_KEY is not set — every adapter arm would skip and the run " +
        "would pass without testing anything. Mint one via e2e/run-e2e.sh or " +
        "set POSTHOG_CODE_E2E_PERSONAL_API_KEY against a reachable POSTHOG_CODE_E2E_GATEWAY_URL.",
    ).toBe(true);
  });
  ...
  it("requires the native codex binary when a token is set (else codex skips-to-green)", () => {
    ...
    expect(
      ...
      "POSTHOG_CODE_E2E_PERSONAL_API_KEY is set but the native codex binary is missing — the " +
