...
# API key (no OAuth), so if POSTHOG_CODE_E2E_PERSONAL_API_KEY is unset this reads the repo's
...
if [[ -z "${POSTHOG_CODE_E2E_PERSONAL_API_KEY:-}" ]]; then
  SETTINGS="$POSTHOG_REPO/ee/settings.py"
  if [[ ! -f "$SETTINGS" ]]; then
    echo "POSTHOG_CODE_E2E_PERSONAL_API_KEY unset and posthog settings not found at $SETTINGS." >&2
    echo "Set POSTHOG_CODE_E2E_PERSONAL_API_KEY, or POSTHOG_REPO to the posthog checkout." >&2
    exit 1
  fi
  # The `llm_gateway` product accepts personal API keys, so no OAuth mint needed.
  POSTHOG_CODE_E2E_PERSONAL_API_KEY="$(grep -E '^DEV_API_KEY[[:space:]]*=' "$SETTINGS" | head -1 | sed -E 's/^DEV_API_KEY[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/')"
fi

if [[ -z "${POSTHOG_CODE_E2E_PERSONAL_API_KEY:-}" ]]; then
  echo "Failed to obtain an POSTHOG_CODE_E2E_PERSONAL_API_KEY (no DEV_API_KEY in ee/settings.py?)." >&2
  echo "If auth then fails, run 'python manage.py setup_local_api_key' in the posthog repo." >&2
  exit 1
fi

export POSTHOG_CODE_E2E_PERSONAL_API_KEY
echo "token: ${POSTHOG_CODE_E2E_PERSONAL_API_KEY:0:8}…  gateway: ${POSTHOG_CODE_E2E_GATEWAY_URL:-http://localhost:3308/llm_gateway}"
cd "$AGENT_DIR"
pnpm test:e2e "$@"
