...
/**
 * ACP SessionConfigSelectOption `_meta` key for the posthog_code free-tier
 * model gate. Adapters mark models the caller's org can't use (the gateway's
 * `allowed: false` annotation) so pickers render them locked behind an
 * upgrade gate instead of omitting them.
 */
