// The freeform (React) canvas authoring contract: output format, the import
// whitelist, the `ph` data shim, and the per-template layout rules. Lives in
// @posthog/shared so both @posthog/core (the canvas UI/templates) and
// @posthog/agent (the canvas_checkout tool) can read it without a package cycle.
import { FREEFORM_WHITELIST } from "./canvas-freeform-whitelist";
...
export const FREEFORM_SYSTEM_PROMPT = buildFreeformPrompt();
