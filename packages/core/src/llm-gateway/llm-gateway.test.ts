...
describe("LlmGatewayService.prompt", () => {
  ...
  const MODEL_GATE_BODY = {
    detail:
      "Model 'claude-haiku-4-5' needs a paid PostHog plan. Models available on the free tier: @cf/zai-org/glm-5.2. Add a payment method to your organization to unlock all models.",
  };
  ...
  it("routes straight to the free-tier model once the org is known unbilled", async () => {
    ...
    // Second call must not burn a 403 round-trip.
    await service.prompt([{ role: "user", content: "hi" }], {
      model: "claude-haiku-4-5",
    });
