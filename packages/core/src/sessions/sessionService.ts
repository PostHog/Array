...
export interface SessionTrpc {
  ...
  checkpoint: {
    replayCheckpoints: TrpcMutation;
    restore: TrpcMutation;
  };
...
export class SessionService {
  ...
  private async reconnectToLocalSession(
    ...
    this.d.trpc.checkpoint.replayCheckpoints
      .mutate({ taskRunId })
      .then(({ count }) => {
        if (count > 0) {
          this.d.log.info("Checkpoint replay triggered after reconnect", {
            taskRunId,
            count,
          });
        }
      ...
      .catch((err) => {
        this.d.log.warn("Failed to replay checkpoints after reconnect", {
          taskRunId,
          error: err,
        });
      });
  ...
  private handleSessionEvent(taskRunId: string, acpMsg: AcpMessage): void {
    ...
    if (
      ...
      if (params?.replay) {
        this.d.log.debug("Skipping checkpoint sync (replayed)", {
          taskRunId,
      ...
      if (params?.checkpointId) {
        ...
        this.d
          ...
          .then(() => {
            this.d.log.info("Checkpoint synced to S3", {
              taskRunId,
              checkpointId: params.checkpointId,
            });
          })
          .catch((error: unknown) => {
            this.d.log.warn("Failed to sync checkpoint to S3", {
              taskRunId,
              checkpointId: params.checkpointId,
              error,
            });
   ...
   */
  /**
   * Revert the working tree + truncate the durable log to a checkpoint (runs the
   * host-side restore mutation). Returns the live session's runtime + whether any
   * log/rollout truncation failed, so the caller can drive the reconnect and warn.
   */
  ...
  async restoreCheckpointReconnect(
    ...
    const resolvedAdapter: Adapter =
      restoredAdapter ??
      this.d.adapterStore.getAdapter(taskRunId) ??
      session.adapter ??
      "codex";
    ...
    this.d.store.updateSession(taskRunId, { adapter: resolvedAdapter });
    this.d.adapterStore.setAdapter(taskRunId, resolvedAdapter);
    ...
    try {
      ...
      if (result) {
        ...
        const reconnected = this.d.store.getSessions()[taskRunId];
        if (reconnected) {
  ...
  public maybeRevertBypassMode(
    ...
    // Revert to the adapter's standard, non-bypass mode. Claude exposes
    // "default"; codex has no "default" (its closest is "auto"). Hardcoding
    // "default" onto a codex session (cloud full-access → local codex after a
    // handoff) throws "Invalid params"; the caller's rollback then re-arms its
    // effect into a retry loop. Resolve against the session's advertised catalog.
    const session = this.d.store.getSessionByTaskId(taskId);
    const modeOpt = getConfigOptionByCategory(session?.configOptions, "mode");
    let revertMode: string | undefined;
    if (modeOpt?.type === "select") {
      const ids = new Set(
        flattenSelectOptions(modeOpt.options).map((o) => o.value),
      );
      if (ids.has("default")) revertMode = "default";
      else if (ids.has("auto")) revertMode = "auto";
    }
    if (revertMode && revertMode !== options.currentModeId) {
      this.setSessionConfigOptionByCategory(taskId, "mode", revertMode);
    }
  }
