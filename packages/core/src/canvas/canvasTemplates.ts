...
const FREEFORM_BASE = [
  ...
  '- `ph.openExternal(url)` asks the host to open an absolute `https://posthog.com` (or `*.posthog.com`) URL — anything else is blocked, so do NOT link to other sites. Use it for links that leave the canvas; sandboxed `target="_blank"` navigation is intentionally blocked.',
