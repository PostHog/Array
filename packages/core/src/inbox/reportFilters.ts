...
/** Count of reports surfaced to the current user as up for review. */
export function countUpForReview(reports: SignalReport[]): number {
  return reports.filter(isReportUpForReview).length;
}

/** Deduped list of enabled source products across the given reports' sources. */
export function deriveEnabledProducts(
  sources: { source_product: string; enabled: boolean }[],
): string[] {
  const enabled = new Set<string>();
  for (const source of sources) {
    if (source.enabled) {
      enabled.add(source.source_product);
    }
  }
