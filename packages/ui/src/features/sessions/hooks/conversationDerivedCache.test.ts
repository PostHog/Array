...
function seedSession(taskId: string, taskRunId: string): void {
...
describe("conversationDerivedCache", () => {
  it("returns the same cache entry across lookups for the same scope + task", () => {
    const key = { scope: "test", taskId: "same-entry" };
  ...
  it("keeps entries of different scopes for the same task independent", () => {
    ...
    const a = getConversationBuildCache({ scope: "thread", taskId });
    const b = getConversationBuildCache({ scope: "footer", taskId });
    expect(a).not.toBe(b);
  });

  it("evicts the least recently used entry beyond the cap", () => {
    const first = getConversationBuildCache({ scope: "lru", taskId: "lru-0" });
    for (let i = 1; i < 50; i++) {
      getConversationBuildCache({ scope: "lru", taskId: `lru-${i}` });
    }
    const recent = getConversationBuildCache({
      scope: "lru",
      taskId: "lru-49",
    });
    expect(getConversationBuildCache({ scope: "lru", taskId: "lru-49" })).toBe(
      recent,
    );
    expect(
      getConversationBuildCache({ scope: "lru", taskId: "lru-0" }),
    ).not.toBe(first);
  });
  ...
  it("drops the cache when the session is removed from the store", () => {
    const key = { scope: "test", taskId: "removed-task" };
  ...
  it("keeps entries for tasks that never had a session through store commits", () => {
    const key = { scope: "test", taskId: "sessionless-task" };
    const before = getConversationBuildCache(key);
