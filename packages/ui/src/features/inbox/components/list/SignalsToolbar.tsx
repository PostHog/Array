...
import { FilterSortMenu } from "@posthog/ui/features/inbox/components/list/FilterSortMenu";
...
export function SignalsToolbar({
  ...
  const fireBulkAction = (
    ...
    for (const event of buildBulkActionEvents(
      actionType,
      targetIds,
      snapshot,
    )) {
      onReportAction(event);
    }
  };

  const handleConfirmDelete = async () => {
    const targetIds = [...effectiveBulkIds];
    const snapshot = snapshotReportList(reports);
