...
This branch is one half of a two-repo change; the companion branch is `posthog-code/agent-run-otel-telemetry` in `PostHog/posthog`.
This repo (`PostHog/code`) carries the telemetry emitter inside the agent; `PostHog/posthog` carries the configuration/injection side.
